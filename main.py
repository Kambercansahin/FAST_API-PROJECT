...
def home(request:Request):
    post_dict = {"posts":posts,"title":"Home"}
    return templates.TemplateResponse(request,"home.html",context=post_dict)


@app.get("/posts/{post_id}",include_in_schema=False,name="post")
def get_unique_post(request:Request,post_id:int):
    for post in posts:
        title = post["title"]
        if post.get("id") ==post_id:
            post_dict = {"post":post,"title":title}
            return templates.TemplateResponse(request,"post.html",context=post_dict)
...
def created_post(post:PostCreate):
